...
func Retry(ctx context.Context, timeout time.Duration, f RetryFunc) error {
...
func (conf *StateChangeConf) WaitForState(ctx context.Context) (interface{}, error) {
	tflog.Debug(ctx, fmt.Sprintf("Waiting for state to become: %s", conf.Target))
	...
	go func() {
		...
		for {
			...
			tflog.Trace(ctx, fmt.Sprintf("Waiting %s before next try", wait))
		}
	}()
	...
	for {
		...
		case <-ctx.Done():
			...
			return nil, ctx.Err()
		case <-timeout:
			tflog.Warn(ctx, fmt.Sprintf("WaitForState timeout after %s", conf.Timeout))
			tflog.Warn(ctx, fmt.Sprintf("WaitForState starting %s refresh grace period", refreshGracePeriod))
		...
		forSelect:
			for {
				...
				case r, ok := <-resCh:
					...
					lastResult = r
				case <-ctx.Done():
					tflog.Error(ctx, "Context cancelation detected, abandoning grace period")
					break forSelect
				case <-timeout:
					tflog.Error(ctx, "WaitForState exceeded refresh grace period")
					break forSelect
				}
			}
